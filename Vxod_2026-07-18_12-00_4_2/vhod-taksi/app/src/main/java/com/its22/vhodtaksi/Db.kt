...
class Db(ctx: Context) : SQLiteOpenHelper(ctx.applicationContext, "vhod.db", null, 2) {
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 2) {
            try { db.execSQL("ALTER TABLE payment ADD COLUMN extra_label TEXT") } catch (e: Exception) {}
            try { db.execSQL("ALTER TABLE payment ADD COLUMN extra_amount REAL NOT NULL DEFAULT 0") } catch (e: Exception) {}
        }
    }
    ...
    fun insert(p: PaymentRecord): Long {
        ...
        cv.put("ts", p.ts)
        cv.put("synced", 0)
        cv.put("extra_label", p.extraLabel)
        cv.put("extra_amount", p.extraAmount)
    ...
    private val cols =
        "id, uuid, apt, name, people, period, amount, personal, elevator, other, ts, synced, extra_label, extra_amount"
