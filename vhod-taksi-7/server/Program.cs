...
WHEN MATCHED THEN UPDATE SET
    ...
    Amount=@Amount, Personal=@Personal, ElevatorShare=@Elevator, OtherShare=@Other,
    ExtraLabel=@ExtraLabel, ExtraAmount=@ExtraAmount, Ts=@Ts, Signature=@Signature
WHEN NOT MATCHED THEN INSERT
    (Uuid, Device, Apt, Name, People, Period, Amount, Personal, ElevatorShare, OtherShare, ExtraLabel, ExtraAmount, Ts, Signature)
    VALUES (@Uuid, @Device, @Apt, @Name, @People, @Period, @Amount, @Personal, @Elevator, @Other, @ExtraLabel, @ExtraAmount, @Ts, @Signature);";
            ...
            var extraLabel = GetStr(r, "extraLabel");
            cmd.Parameters.AddWithValue("@ExtraLabel", string.IsNullOrEmpty(extraLabel) ? (object)DBNull.Value : extraLabel);
            cmd.Parameters.AddWithValue("@ExtraAmount", GetDec(r, "extraAmount"));
