...
object Receipts {
    ...
    fun payment(
        ...
        if (apt.paysElevator && due.elevatorShare > 0.0) {
            ...
            if (elevNames.isNotBlank()) L.add(Escpos.Line("   вкл.: " + elevNames, size = 19f))
        }
        L.add(
            Escpos.Line(
        ...
        if (otherNames.isNotBlank()) L.add(Escpos.Line("   вкл.: " + otherNames, size = 19f))
        if (extraAmount > 0.0 || extraLabel.isNotBlank()) {
            val lbl = if (extraLabel.isNotBlank()) extraLabel else "Допълнително"
            L.add(Escpos.Line(text = lbl + ":", rightText = money2(extraAmount) + " " + cur, size = 24f, bold = true))
        }
        L.add(Escpos.Line(separator = true, size = 24f))
        L.add(Escpos.Line(text = "ОБЩО:", rightText = money2(due.total + extraAmount) + " " + cur, size = 34f, bold = true, extra = 14f))
        L.add(Escpos.Line(separator = true, size = 24f))
        if (signature != null) {
